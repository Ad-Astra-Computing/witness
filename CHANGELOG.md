...
- `parseCheckpoint` enforces the 1024-character body and 256-character line caps before splitting, and validates the origin as printable ASCII with no spaces. An origin containing a space produces a signature line (`-- <origin> <signature>`) that no verifier can parse, so `WITNESS_ORIGIN` is now validated the same way at Durable Object startup rather than yielding an unverifiable log.
