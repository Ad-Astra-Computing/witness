...
- **Breaking.** Numbers in a signed body are now restricted to the INK safe-integer profile: an integer that round-trips exactly through an IEEE-754 double, excluding negative zero. The witness previously accepted any finite value whose shortest form avoided exponential notation, which admitted fractions (`0.95`), integers past 2^53, and magnitudes up to 1e20. Those values reached the Merkle tree and received signed inclusion receipts that an INK verifier could not recompute, because the leaf hash is taken over canonical JSON and serializers disagree on exactly those values. Submissions carrying one are now refused at the door.
- Signature-bearing request bodies pass a byte-level gate before parsing: a fatal UTF-8 decode (an invalid sequence is refused rather than becoming U+FFFD), a lone UTF-16 surrogate escape scan, and a scan for number literals outside the IEEE-754 double range. The transport signature covers the raw bytes, so a lenient decode would canonicalize something other than what the sender signed. Rejections return HTTP 400 with `invalid_signed_body: utf8 | surrogate | number-range`.
- Canonicalization is bounded before it runs: 32 levels of nesting (was 64), 10,000 nodes, 1.2M aggregate characters of keys and string values, and a 1 MB cap on the canonical output.
- Timestamps in the transport signature base and on the submitted audit event are parsed with a strict RFC 3339 grammar instead of `new Date`. Transport auth runs ahead of schema validation, so the lenient parse was the operative rule: it accepted date-only and zone-less values and silently rolled `2026-02-29` over to March 1. A body that fails canonicalization during signature-base construction now returns `uncanonicalizable_body` (HTTP 401) rather than a 500.
- `parseCheckpoint` enforces the 1024-character body and 256-character line caps before splitting, and otherwise accepts any non-empty origin, which is the whole of the INK profile rule. An origin containing a space produces a signature line (`-- <origin> <signature>`) that no verifier can parse, so `WITNESS_ORIGIN` is validated at Durable Object startup against a stricter rule: printable ASCII, no spaces. Which origin a witness publishes under and which bodies it will parse are separate questions, and answering the second one strictly made this parser reject non-ASCII origins every other implementation accepts.
- New `test/ink-parity.test.ts` pins these rules as fixed accept/reject tables and literal leaf-hash digests, so a future divergence fails a test instead of passing a self-consistent round trip.
- A slice of the INK conformance corpus is vendored under `conformance/` from the published `@adastracomputing/ink` tarball and run against the witness by `test/conformance.test.ts`: 136 cases across the signed-body gate, the JCS number and string profiles, timestamps, checkpoints, Merkle leaves and inclusion proofs. The verdicts come from outside this repository, which is what caught the checkpoint origin rule above. `src/conformance/decide.ts` names the categories the witness does not decide and why, so a category added upstream fails a test.
